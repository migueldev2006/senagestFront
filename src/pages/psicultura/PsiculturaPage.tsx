...
const refrescarToggle = (registros: any[]) => {
  ...
  // Buscar manuales activos
  const manualAbierto = registros.find(
    r => r.modo === "manual" && (!r.fin || r.fin === null)
  );

  if (manualAbierto) {
    console.log("🔎 Toggle basado en MANUAL:", manualAbierto);
    return manualAbierto.estado === true;
  }

  // Si no hay manual → tomar el registro automático virtual
  const auto = registros.find(r => r.estadoActual === "automatico");
  if (!auto) return false;

  console.log("🔎 Toggle basado en AUTOMÁTICO:", auto);
  return auto.estado === true;
};
